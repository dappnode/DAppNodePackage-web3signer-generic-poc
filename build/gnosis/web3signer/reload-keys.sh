...
function send_dappmanager_notification() {
  curl -X POST -G 'http://my.dappnode/notification-send' --data-urlencode 'type=danger' --data-urlencode title="$_DAPPNODE_GLOBAL_CONSENSUS_CLIENT_GNOSIS is not available" --data-urlencode 'body=Make sure you select an available client in the web3signer at packages > web3signer gnosis > config > gnosis Beacon Chain Consensus Layer Client'
}
