...
function ensure_requirements() {
  ...
  if [ "$(curl -s -X GET \
    ...
    -H "Host: prysm.migration${_BUILD_API_NETWORK}.dappnode" \
    --write-out '%{http_code}' \
    --silent \
    --output /dev/null \
    --retry 60 \
...
function import_validators() {
  import-one-by-one --keystores-path "$MANUAL_MIGRATION_DIR" --wallet-password-path "$BACKUP_WALLETPASSWORD_FILE" --network ${_BUILD_NETWORK}
