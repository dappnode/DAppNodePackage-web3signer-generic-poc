...
for client in "${CLIENTS_TO_REMOVE[@]}"; do
  case "$client" in
  "prysm")
    CLIENT_API="http://validator.prysm${_BUILD_API_NETWORK}.dappnode:3500"
    TOKEN_FILE="/security/prysm/auth-token"
    CERT_REQUEST=""
    ;;
  "teku")
    CLIENT_API="https://validator.teku${_BUILD_API_NETWORK}.dappnode:3500"
    ...
    ;;
  "lighthouse")
    CLIENT_API="http://validator.lighthouse${_BUILD_API_NETWORK}.dappnode:3500"
    TOKEN_FILE="/security/lighthouse/api-token.txt"
    CERT_REQUEST=""
    ;;
  "nimbus")
    CLIENT_API="http://beacon-validator.nimbus${_BUILD_API_NETWORK}.dappnode:3500"
